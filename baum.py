...
def backward():
    # node values stored during forward algorithm
    backward_vals = np.zeros((len(states), len(test_sequence)))
    ...
    for i in range(1,len(test_sequence)+1):
        for j in range(len(states)):
            # if first sequence value then do this
            if (-i == -1):
                backward_vals[j, -i] = end_probs[j]
            # else perform this
            else:
                values = [backward_vals[k, -i+1] * emission[k, sequence_syms[test_sequence[-i+1]]] * transition[j, k] for k in range(len(states))]
                backward_vals[j, -i] = sum(values)

    #start state value
    start_state = [backward_vals[m,0] * emission[m, sequence_syms[test_sequence[0]]] for m in range(len(states))]
    start_state = np.multiply(start_state, start_probs)
    start_state_val = sum(start_state)
    return backward_vals, start_state_val
...
for iteration in range(2000):
    ...
    fwd_probs, fwd_val = foward()
    bwd_probs, bwd_val = backward()
    ...
    new_fwd_temp, new_fwd_temp_val = foward()
